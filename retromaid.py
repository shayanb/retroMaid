...
class RetroMaid:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.scraper = ScreenScraper(scraper_config)
            ...
            for rom in missing:
                ...
                try:
                    ...
                    if rom.hash_info and rom.hash_info.get('md5'):
                        game = self.scraper.search_by_hash(
                            rom.hash_info['md5'],
                            'md5',
                            system,
                    ...
                    if not game:
                        ...
                        results = self.scraper.search_by_name(clean_name, system, region)
                        ...
                        if results:
                            ...
                            if game.confidence < 80:
                                choice = self._confirm_game_match(rom.filename, game)
                                if not choice:
                    ...
                    console.print(f"[green]✓[/green] {game.name}")
    ...
    def _confirm_game_match(self, rom_filename: str, game) -> bool:
        """Ask user to confirm a game match"""
        console.print(f"\n[yellow]Low confidence match for:[/yellow] {rom_filename}")
        console.print(f"Found: {game.name} (confidence: {game.confidence:.0f}%)")

        if game.description:
            console.print(f"Description: {game.description[:200]}...")

        return Confirm.ask("Is this correct?", default=True)
